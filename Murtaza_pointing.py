...
while True:
    ...
    if hands and not buttonPressed:  # If hand is detected
        ...
        indexFinger = getImageXYFromZ((indexTipX, indexTipY, indexTipZ), (indexMcpX, indexMcpY, indexMcpZ))
        print(indexFinger)

        pointerColor = cv2.cvtColor(np.array([[[abs(indexTipZ), 255, 255]]]).astype(np.uint8), cv2.COLOR_HSV2BGR)
